...
extern "C" {
	...
	void escutaPipes(SincPipes& sincPipes) {
		...
		while (!sincPipes.termina && sincPipes.numClientes < N_PIPES) {
			...
			_tprintf(TEXT("%s: Esperar liga��o de um jogador...\n"), SERVIDOR);
	...
	bool AcessoPipesMensagensServidor(SincPipes& sincPipes) {
		...
		for (i = 0; i < N_PIPES; i++) {
			_tprintf(TEXT("%s: (CreateNamedPipe [%d]) Criar uma c�pia do pipe '%s' \n"), SERVIDOR, i, PIPE_MENSAGENS);
			...
			_tprintf(TEXT("%s: (ConnectNamedPipe [%d]) Esperar liga��o overlapped de um jogador...\n"), SERVIDOR, i);

			if (ConnectNamedPipe(sincPipes.hPipes[i].hInstance, &sincPipes.hPipes[i].overlap) != 0) {
				_tprintf(TEXT("%s: (ConnectNamedPipe [%d]) ERRO [%d] ao estabelecer liga��o ao jogador!\n"), SERVIDOR, i, GetLastError());
				break;
			}
